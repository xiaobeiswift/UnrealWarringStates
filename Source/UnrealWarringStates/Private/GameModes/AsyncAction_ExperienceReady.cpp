...
UAsyncAction_ExperienceReady::UAsyncAction_ExperienceReady(const FObjectInitializer& ObjectInitializer) // AsyncAction_ExperienceReady的构造函数
	: Super(ObjectInitializer) // 调用基类的构造函数
{
}

UAsyncAction_ExperienceReady* UAsyncAction_ExperienceReady::WaitForExperienceReady(UObject* WorldContextObject) // 生成等待经验准备的异步动作
{
	UAsyncAction_ExperienceReady* Action = nullptr; // 创建一个异步动作对象

	if (UWorld* World = GEngine->GetWorldFromContextObject(WorldContextObject, EGetWorldErrorMode::LogAndReturnNull)) // 从上下文对象获取世界对象
	{
...
void UAsyncAction_ExperienceReady::Step2_ListenToExperienceLoading(AGameStateBase* GameState) // 监听经验加载过程
{
	...
	UWSExperienceManagerComponent* ExperienceComponent = GameState->FindComponentByClass<UWSExperienceManagerComponent>(); // 查找游戏状态对象上的WSExperienceManagerComponent组件
	...
	if (ExperienceComponent->IsExperienceLoaded()) // 如果体验已经加载完成
	{
		...
		World->GetTimerManager().SetTimerForNextTick(FTimerDelegate::CreateUObject(this, &ThisClass::Step4_BroadcastReady)); // 设置在下一帧触发的定时器，以传播经验准备消息
	}
	else
	{
		ExperienceComponent->CallOrRegister_OnExperienceLoaded(FOnWSExperienceLoaded::FDelegate::CreateUObject(this, &ThisClass::Step3_HandleExperienceLoaded)); // 调用或注册经验加载完成回调函数
	}
}

void UAsyncAction_ExperienceReady::Step3_HandleExperienceLoaded(const UWSExperienceDefinition* CurrentExperience) // 处理经验加载完成事件
{
